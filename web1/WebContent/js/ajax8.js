...
function goFunc(element){
	...
	xhr.send("buser=" + ele.id); //get도 이렇게 담겨서 가지만 url 파일명뒤에 붙어서 간다
}
...
function setOffsets() {
	...
	/*
	 * 부모노드 폭, offsetHeight:높이 offsetWidth 속성 : offsetParent 설정에 따라 모체의 레이아웃이나
	 * 좌표에 상대적으로 계산된 너비 위치를 픽셀단위로 반환한다.
	 */
...
function calcOffset(field, attr) {
	...
	while (field) {
		...
		/*
		 * offsetTop 속성 : offsetParent 설정에 따라 모체의 레이아웃이나 좌표에 상대적으로 계산된 수직 위치를
		 * 픽셀단 위로 반환한다.
		 */
	}
	return offset;
